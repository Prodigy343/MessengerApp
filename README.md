...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create messenger-app-5c3c
  fly apps create messenger-app-5c3c-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app messenger-app-5c3c
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app messenger-app-5c3c-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'messenger-app-5c3c-staging' does not match app name in config file 'messenger-app-5c3c'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name messenger-app-5c3c-db
  fly postgres attach --app messenger-app-5c3c messenger-app-5c3c-db

  fly postgres create --name messenger-app-5c3c-staging-db
  fly postgres attach --app messenger-app-5c3c-staging messenger-app-5c3c-staging-db
  ```
